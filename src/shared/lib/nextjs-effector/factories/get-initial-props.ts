...
import type { AnyProps, EmptyOrPageEvent, GetInitialProps } from "../types";
...
export function createGIPFactory({
  ...
  return function createGIP<P extends AnyProps = AnyProps>({
    ...
    return async function getInitialProps(context) {
      ...
      const factoryGipResult =
        (await factoryCustomize?.({ scope, context })) ?? ({} as P);
      const pageGipResult =
        (await pageCustomize?.({ scope, context })) ?? ({} as P);
