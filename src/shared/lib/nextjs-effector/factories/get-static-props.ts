...
import type { AnyProps, EmptyOrStaticPageEvent } from "../types";

export type GetStaticPropsExitResult =
  | { redirect: Redirect; revalidate?: number | boolean }
  | { notFound: true; revalidate?: number | boolean };
export type GetStaticPropsResultWithoutExit<Props> = Exclude<
  GetStaticPropsResult<Props>,
  GetStaticPropsExitResult
>;
  ...
  > {
    return async function getStaticProps(context) {
      ...
      if (["notFound", "redirect"].some((v) => v in gspResult)) {
        ...
        (gspResult as GetStaticPropsResultWithoutExit<P>).props =
          (gspResult as GetStaticPropsResultWithoutExit<P>).props ?? ({} as P);
      }
